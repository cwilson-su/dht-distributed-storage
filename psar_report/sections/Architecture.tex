...
Contrairement \`a Chord, dont l'organisation repose sur un anneau circulaire strict et un routage bas\'e sur des sauts exponentiels via la finger table, Pastry adopte une structure fond\'ee sur le routage par pr\'efixes dans un espace d'identifiants structur\'e en base $2^b$.

Chaque n\oe ud poss\`ede un identifiant unique appel\'e \emph{nodeId}, g\'en\'eralement cod\'e sur 128 bits, issu d'une fonction de hachage cryptographique. De la m\^eme mani\`ere, chaque cl\'e associ\'ee \`a une donn\'ee est transform\'ee en identifiant dans le m\^eme espace. L'objectif du protocole est d'acheminer toute requ\^ete vers le n\oe ud dont l'identifiant est num\'eriquement le plus proche de la cl\'e cible.

L'architecture Pastry repose sur trois structures fondamentales maintenues localement par chaque n\oe ud :

\begin{itemize}
    \item une Routing Table, organis\'ee par longueur de pr\'efixes communs,
    \item un Leaf Set, contenant les identifiants imm\'ediatement sup\'erieurs et inf\'erieurs,
    \item un Neighbor Set, optimisant la proximit\'e r\'eseau physique.
\end{itemize}
